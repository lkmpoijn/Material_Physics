\chapter{相变}
    ...
    \section{马氏体相变}
        ...
        由于马氏体相变的应用不仅可以使钢强化，而且还在材料的增韧、材料功能（如阻尼、介电性）的开发等方面有颇
        ...
        \subsection{马氏体相变的动力学}
            ...
            \subsubsection{马氏体相变转变类型}
                \textbf{马氏体相变转变类型}\index{马氏体相变转变类型}有三种：
                \begin{itemize}
                    \item[1] \textbf{变温转变}\index{马氏体相变转变类型!变温转变}：降温过程中瞬时形核、瞬时长大，转变量与时间无关，只取决于转变温度，如碳钢、合金钢；
                    \item[2] \textbf{等温转变}\index{马氏体相变转变类型!等温转变}：等温形核、瞬时长大，转变量取决于时间，如无碳的\ce{Fe-Ni-Mn}等；
                    \item[3] \textbf{爆发转变}\index{马氏体相变转变类型!爆发转变}：对于$M_s<\SI{0}{\celsius}$的合金，冷到$M_s$以下的温度书简，剧烈的形成大量马氏体，随后变温长大，转变时伴有声音并放出大量的相变热。
                \end{itemize}
            \subsubsection{形核长大}
        ...
        \subsection{马氏体可逆转变和形状记忆合金}
            ...
            \textbf{逆转变}\index{逆转变}或\textbf{反相变}\index{反转变|seealso{逆转变}}。

            发生马氏体相变和逆转变中存在部分不可逆的能量，这一部分称为热滞，其中一些
            合金在冷却转变和加热相变完全没有热滞和热滞很小，从而使马氏体呈现弹性似
            的长大和收缩，所以称为\textbf{热弹性马氏体}\index{热弹性马氏体}。
            ...
            \subsubsection{伪弹性马氏体}
                ...
                若应力诱发马氏体相变的逆转变滞后，当外加应力降低到零时，不能完全逆转变，则剩余的马氏体可以通
        ...
        \subsection{陈化稳定}
            ...
            氏体“稳定”、较难转变为马氏体的现象，叫做\textbf{陈化稳定}\index{陈化稳定}
    ...
    \section{共析转变}
        ...
        \begin{equation}
            \gamma\to\alpha+\beta,
        \end{equation}
        这一过程为\textbf{共析转变}\index{共析转变}，以相互协作的方式从固溶体中同时析出结构、成分均不
        同于母相的两个或两个以上的固相的转变过程，比如钢中发生的珠光体转变就是一个典型的共析转变。

        \subsection{亚共析分解}
            以\ce{Fe-C}相图为例，如\autoref{FeC相图}所示，在共析点左侧的钢称为亚共析钢，
            \begin{figure}[ht]
                \centering
                \includegraphics[width=0.7\textwidth]{fig/Fe-C.jpg}
                \caption{\ce{Fe-C}相图及各处组织形貌。}
                \label{FeC相图}
            \end{figure}
        ...
        \subsection{共析分解}
            对于共析钢来说，共析分解的特点为
            \begin{itemize}
                \item[1] 分解包括两个过程：\ce{C}的重新分布和晶体学上的变化即结构改变，结构改变必须在\ce{C}的重新分布的基础上进行；
                \item[2] 共析转变先析出\ce{Fe_3C}在其周围(两侧)形成贫碳的$\alpha-\ce{Fe}$，再在外侧析出\ce{Fe3C}，交替进行，有一个珠光体核长大的平行片区称为珠光体领域或珠光体胞；
